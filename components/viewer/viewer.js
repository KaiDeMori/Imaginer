...
export class Viewer {
  ...
  async open(blob, opts = {}) {
    ...
    this.mask_cache_dirty = true;
    if (opts.image_id && window.database_store) {
      try {
        const rec = await window.database_store.get(opts.image_id);
  ...
  async close() {
    ...
    if (this.mask_data && this.bitmap && this.image_id && window.database_store) {
      ...
      let rec = await window.database_store.get(this.image_id);
      ...
      if (!is_empty) {
        ...
        await window.database_store.update(this.image_id, { mask_blob, uuid });
        ...
        await window.database_store.update(this.image_id, { mask_blob: null });
