...
export class Viewer {
        ...
        if (opts.image_id && window.sessionStore) {
            try {
                ...
                if (
                    rec &&
                    rec.mask_blob &&
                    typeof rec.mask_blob === 'object' &&
                    rec.mask_blob instanceof Blob &&
                    rec.mask_blob.size > 0
                ) {
    ...
    /* ---------------- Drawing Helpers -------------------------------- */
    compute_fit_scale() {
        if (!this.bitmap) { this.fit_scale = 1; return; }
        const maxW = window.innerWidth  * 0.9;   // 10 % margin
        const maxH = window.innerHeight * 0.9;
        this.fit_scale = Math.min(maxW / this.bitmap.width,
                                 maxH / this.bitmap.height,
                                 1);
    }
