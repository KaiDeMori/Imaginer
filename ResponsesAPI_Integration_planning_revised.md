...
4.  **Streaming**: The Responses API supports streaming. Implementing this would make the UI feel much snappier.

### Next Steps
1.  Create the `api/responses_client.js` wrapper.
2.  Implement the mode switch logic in `app.js`.
3.  Build the `Conversation_panel` skeleton with the specified structure (History -> Input -> Prompt).
