...
- If models are missing, use Config → Account → **Refresh Image Models**. Older models and dated snapshots are listed only with Config → Account → **Show older models** enabled.
...
- **Why does the model dropdown show only two models?** By default the dropdown lists the two recommended models, `gpt-image-2.5-flare` and `gpt-image-2.5-sunburst`. Enable Config → Account → **Show older models** to list every image model of your account.
...
- **My selected model changed to `gpt-image-2.5-flare`. Why?** Disabling Config → Account → **Show older models** while an older model is selected switches the selection to `gpt-image-2.5-flare`. Enable the setting again to select an older model.
