/*--------------------------------------------------------------------
   ...
   --------------------------------------------------------------------
   General-purpose WebGL region zoom/warp rendering helper. All WebGL setup
   happens once in create_image_region_zoom_utils_render(…), then every animation
   ...
   Signature
      const zoomer = create_image_region_zoom_utils_render(canvas, gl, img, fromRect, toRect);
...
function create_image_region_zoom_utils_render(canvas, gl, img, fromRect, toRect) {
  // ---------------------------------------------------------------
  ...
  function make_rect_square(rect) {
    ...
    const dx1 = rect.p1.x - rect.p0.x,
      dy1 = rect.p1.y - rect.p0.y;
    const dx3 = rect.p3.x - rect.p0.x,
      dy3 = rect.p3.y - rect.p0.y;
  ...
  function rect_to_axes(rect) {
    ...
    const msg = `[RegionZoom] rect_to_axes: o=(${fmt(o)}), u=(${fmt(u)}), v=(${fmt(v)}), |u|=${u_len.toFixed(4)}, |v|=${v_len.toFixed(4)}, aspect=${(
      u_len / v_len
    ).toFixed(4)}`;
    console.log(msg);
    return { o, u, v };
...
window.create_image_region_zoom_utils_render = create_image_region_zoom_utils_render;
