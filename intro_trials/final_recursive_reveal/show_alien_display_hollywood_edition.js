/*--------------------------------------------------------------------
  show_alien_display_hollywood_edition.js
  --------------------------------------------------------------------
  Hollywood-style edition of the “alien display” helper.  All WebGL
  setup happens once in create_alien_display(…), then every animation
  frame you simply call alien.draw(t) with a 0‒1 parameter.

  Signature
    const alien = create_alien_display(canvas, gl, img, fromRect, toRect);

      canvas   – <canvas> element that already lives in the DOM.
      gl       – WebGLRenderingContext.
      img      – <img> (fully loaded) that will become the texture.
      fromRect – { p0:{x,y}, p1:{x,y}, p3:{x,y} }  keyframe at t = 0.
      toRect   – { p0:{x,y}, p1:{x,y}, p3:{x,y} }  keyframe at t = 1  (optional, defaults → fromRect).

    alien.draw(t)  – render with interpolation factor t ∈ [0,1].
--------------------------------------------------------------------*/
...
function create_alien_display(canvas, gl, img, fromRect, toRect) {
  ...
  const r0 = rect_to_axes(fromRect);
  const r1 = rect_to_axes(toRect || fromRect);
  ...
  // ---------------------------------------------------------------
  function draw(t) {
    const o = mix2(r0.o, r1.o, t);
    const u = mix2(r0.u, r1.u, t);
    const v = mix2(r0.v, r1.v, t);
