...
def stack_images_from_dir(base_dir):
    file_list_file = os.path.join(base_dir, "GIMP_infinite_zoom_sequence_files.txt")
    if not os.path.isfile(file_list_file):
        pdb.gimp_message("GIMP_infinite_zoom_sequence_files.txt not found in directory: {}".format(base_dir))
        return
    png_files = read_file_list(file_list_file)
    if not png_files:
...
def infinite_zoom_project_manager(mode, base_dir, png1, png2, png3, png4):
    if mode == 0:
        # Open existing project
        stack_images_from_dir(base_dir)
    else:
...
register(
    ...
    [
        ...
        (PF_STRING, "base_dir", "Base directory for project (for open)", ""),
        (PF_FILE, "png1", "PNG file 1 (sets base directory, for create)", None),
        (PF_FILE, "png2", "PNG file 2 (for create)", None),
        (PF_FILE, "png3", "PNG file 3 (for create)", None),
        (PF_FILE, "png4", "PNG file 4 (for create)", None),
    ],
    [],
